...
class InsuranceRAGSystem:
    ...
    def _check_vector_db_status(self) -> bool:
        ...
        try:
            ...
            if total_chunks > 0:
                logger.info(f"Vector database has {total_chunks} chunks")
                return True
            else:
    ...
    def has_data(self) -> bool:
        """Check if the system has data to query"""
        return self._check_vector_db_status()
    ...
    def _store_chunks(self, chunks: List[Dict[str, Any]], source_file: str) -> None:
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = {
                ...
                'has_table_data': chunk.get('features', {}).get('has_table_data', False),
                'has_legal_terms': chunk.get('features', {}).get('has_legal_terms', False)
            }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # Get actual data from vector database
            collection_data = self.collection.get()
            total_chunks = len(collection_data['ids']) if collection_data['ids'] else 0
            ...
            stats = {
                ...
                'processed_files': list(source_files),
                'vector_db_size': total_chunks
            }
            
            # Add per-file statistics if we have processed_docs cache
